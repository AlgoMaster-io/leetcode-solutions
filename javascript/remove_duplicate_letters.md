...
class Solution {
    removeDuplicateLetters(s) {
        let lastOccurrence = new Array(26).fill(0); // Store last index of each character
        let inStack = new Array(26).fill(false); // Track if character is already in the stack
        ...
        for (let i = 0; i < s.length; i++) {
            ...
            if (inStack[c - 'a'.charCodeAt(0)]) {
                continue;
            }

            // Remove characters from the stack that are greater than the current character
            // and can appear later in the string
            while (stack.length > 0 && stack[stack.length - 1] > c && lastOccurrence[stack[stack.length - 1] - 'a'.charCodeAt(0)] > i) {
                inStack[stack.pop() - 'a'.charCodeAt(0)] = false;
            }

            // Push the current character onto the stack
            stack.push(c);
            inStack[c - 'a'.charCodeAt(0)] = true;
        }
