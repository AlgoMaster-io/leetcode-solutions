...
// Space Complexity: O(1)
function maxNumberOfBalloons(text: string): number {
    const charCount: number[] = new Array(26).fill(0); // Frequency array for all characters
    ...
    const l = Math.floor(charCount['l'.charCodeAt(0) - 'a'.charCodeAt(0)] / 2); // 'l' appears twice
    const o = Math.floor(charCount['o'.charCodeAt(0) - 'a'.charCodeAt(0)] / 2); // 'o' appears twice
...
// Space Complexity: O(1)
function maxNumberOfBalloons(text: string): number {
    const counts: number[] = new Array(5).fill(0); // Indices: 0->b, 1->a, 2->l, 3->o, 4->n
