...
function reverseBetween(head: ListNode | null, left: number, right: number): ListNode | null {
    ...
    let current: ListNode | null = prev.next; // The first node to be reversed
    let next: ListNode | null = null; // Temporary node to store the next node
    let prevSublist: ListNode | null = null; // Tracks the reversed sublist
