...
class Solution {
    searchRange(nums: number[], target: number): number[] {
        const first = this.findPosition(nums, target, true);  // Find first occurrence
        const last = this.findPosition(nums, target, false); // Find last occurrence
        return [first, last];
    }

    private findPosition(nums: number[], target: number, findFirst: boolean): number {
