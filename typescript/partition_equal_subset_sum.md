...
class Solution {
    canPartition(nums: number[]): boolean {
        ...
        return this.canPartitionHelper(nums, sum / 2, 0);
    }
    
    private canPartitionHelper(nums: number[], target: number, index: number): boolean {
        // Base cases
        if (target === 0) return true;
        if (target < 0 || index >= nums.length) return false;
        
        // Recursive call to include or exclude the current element
        return this.canPartitionHelper(nums, target - nums[index], index + 1) || 
               this.canPartitionHelper(nums, target, index + 1);
    }
}
```
...
class Solution {
    canPartition(nums: number[]): boolean {
        ...
        const dp: boolean[][] = Array.from({ length: nums.length + 1 }, () => Array(target + 1).fill(false));
