...
class Solution {
    ...
    private calculate(grid: number[][], i: number, j: number): number {
        // Base cases for boundaries
        if (i === 0 && j === 0) return grid[0][0]; // Start point
        if (i < 0 || j < 0) return Number.MAX_SAFE_INTEGER; // Out of grid
...
class Solution {
    minPathSum(grid: number[][]): number {
        const memo: number[][] = Array.from({ length: grid.length }, () => new Array(grid[0].length).fill(0));
    ...
    private calculate(grid: number[][], i: number, j: number, memo: number[][]): number {
        // Base cases for boundaries
        if (i === 0 && j === 0) return grid[0][0]; // Start point
        if (i < 0 || j < 0) return Number.MAX_SAFE_INTEGER; // Out of grid
