...
class Solution {
    ...
    private search(nums: number[], start: number, end: number): number {
        if (start === end) {
        ...
        if (nums[mid] < nums[mid + 1]) {
            return this.search(nums, mid + 1, end); // Recur for the right half
        } else {
            return this.search(nums, start, mid); // Recur for the left half
        }
    }
}
```
