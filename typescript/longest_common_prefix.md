...
class Solution {
    public longestCommonPrefix(strs: string[]): string {
        ...
        return this.longestCommonPrefixUtil(strs, 0, strs.length - 1);
    }

    private longestCommonPrefixUtil(strs: string[], left: number, right: number): string {
        if (left === right) {
        ...
        const lcpLeft = this.longestCommonPrefixUtil(strs, left, mid);
        const lcpRight = this.longestCommonPrefixUtil(strs, mid + 1, right);
...
class Solution {
    ...
    private isCommonPrefix(strs: string[], len: number): boolean {
